...
import { RefreshCw, BriefcaseBusiness } from 'lucide-react';
...
const AdminJobApplications: React.FC = () => {
  ...
  } = useAdminInbox('job_applications', {
    searchColumns: ['name', 'email', 'phone', 'cover_letter'],
  });
  const apps = rows as JobApp[];

  const updateStatus = async (id: string, status: string) => {
    const previous = apps;
    setRows((prev) => (prev as JobApp[]).map((a) => (a.id === id ? { ...a, status } : a)) as typeof prev);
  ...
  const deleteApp = async (id: string) => {
    const previous = apps;
    setRows((prev) => (prev as JobApp[]).filter((a) => a.id !== id) as typeof prev);
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardContent className="p-4">
          <InboxFilters
            ...
            searchPlaceholder="Search name, email, phone, cover letter…"
            total={total}
            showing={apps.length}
          />
        </CardContent>
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader>
          <CardTitle className="text-white flex items-center gap-2 text-base">
            <BriefcaseBusiness className="w-4 h-4" /> Submissions
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <SubmissionList
            rows={apps.map((a) => ({
              ...
              body: a.cover_letter,
              link: a.portfolio_url
                ? { label: 'Portfolio', url: a.portfolio_url }
                : a.resume_url
                  ? { label: 'Resume', url: a.resume_url }
                  : null,
            }))}
            loading={loading}
            error={error}
            hasActiveFilters={hasActiveFilters}
            emptyLabel="No job applications yet."
